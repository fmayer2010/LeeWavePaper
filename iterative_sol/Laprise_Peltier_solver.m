...
J=.05; % Juice number (internal froude number squared, N*h/U) 
h_0=J*U/N; % mximum height of bathymetry
kg = N/U; % lee wave wavenumber
Lg = 2*pi/kg; % lee wave wavelength

epsilon = 0.01; % launching number = nonhydrostatic number = U*kh/N
...
%% toggle bathymetry, plotting, and figure saving
% enter 1 in the element of the bathy-vector over which you want to flow
% bathy=[sine, witch,gaussian, semi-elipse, backward step, foreward step, rectangle, spectral]
bathy=zeros(1,8);
bathy(8)=1;
% if choose spectral bathymetry, must give std deviation of phase.  
% stdphi>1/2 give goff style abyssal hills. 
% stdphi<=0.25 gives a nice seamount
stdphi=.1;

% enter 1 in each element of the plot-vector to get that plot
...
% toggle saving .mat file with velocity vectors, dx, and dz
saving = 0;

%% grid
% 
% % sine grid (only 4 wavelength long... not long enough to resolve other
% bathymetries in fourier space
if bathy(1)==1
...
% % all other grids (30 wavelengths long)
else
...
elseif bathy(4)==1
% semi-elipse
    h = h_0/Lx.*sqrt(Lx^2-x.^2);
elseif bathy(5)==1
% backward step
%    h=h_0/2.*(x+abs(x))./x;
    h = (h_0+h_0.*tanh(10.*x./Lx))./2;
elseif bathy(6)==1
% forward step
%    h=h_0/2.*(x+abs(x))./x;
    h = (h_0+h_0.*tanh(10.*-x./Lx))./2;
elseif bathy(7)==1
% rectangle
    h = h_0.*(tanh(x+Lx)+tanh(-x+Lx));
elseif bathy(8)==1
% spectral
    h=spectralbathy(h_0,kh,Nx,stdphi);
    hmax = (max(h)-min(h))/2;
    Jmax = N*hmax/U
    z = linspace(min(h),1.5*Lg,Nz); % height of plotted domain.
    dz = z(2)-z(1);
    zz_star = zz./Lg;
    z_star = z./Lg;
    dz_star = dz/Lg;
...
% % later on... change the figure call to an integer>sum(plotting) to keep the plot
% figure(1)
% plot(k,abs(eta_hat))
...
% To evaluate at z=h(x), must loop through each k, determine if m(k) is evanescent
% or propagating, and then evaluate eta_hat with m(k) at each h(x). This generates
% eta_hat_h(k,h(x))... thus a matrix of size (Nx,Nx)
...
% update Error, old_error, and iter
    ...
    if max_error>old_error
        blowup_loc = find(abs(Enorm)==max_error);
        disp(['WARNING: solution blowing up. Caught at iter=',...
            num2str(iter),', eta_0 last updated for iter=',...
            num2str(iter-1)])
        disp(['max_error=',num2str(old_error),' at x/L=',...
            num2str(xx_star(blowup_loc)),', z/Lg=',num2str(zz_star(blowup_loc))])
    end
end
...
if plotting(1)==1
    ...
    hold on
        fill([x_star(1),x_star, x_star(end)],[z(1),h,z(1)]./(2*pi*U/N),'k')
    hold off
...
if plotting(2)==1
    ...
    hold on
        fill([x_star(1),x_star, x_star(end)],[z(1),h,z(1)]./Lg,'w')
    hold off
    title(['$u_{total}/(U)$ via diff($\Psi$), $J$=',num2str(J),...
        ', $\epsilon$=',num2str(epsilon)],'Interpreter','latex')
...
if plotting(3)==1
    ...
    hold on
        fill([x_star(1),x_star, x_star(end)],[z(1),h,z(1)]./Lg,'w')
    hold off
    title(['$\rho/\rho_0$ via $\Psi$, $J$=',num2str(J),...
        ', $\epsilon$=',num2str(epsilon)],'Interpreter','latex')
    ylabel('z/Lc','Interpreter','latex')
    set(gca,'XTickLabel','')
    colorbar
...
if plotting(4)==1
    ...
    hold on
        fill([x_star(1),x_star, x_star(end)],[z(1),h,z(1)]./Lg,'w')
    hold off
    title(['$p/(\rho U^2)$ via diff($\eta$), $J$=',num2str(J),...
        ', $\epsilon$=',num2str(epsilon)],'Interpreter','latex')
    set(gca,'XTickLabel','')
    colorbar
    subplot(3,2,[3,4])
...
if plotting(5)==1
    ...
    title({['$\partial \eta/\partial z$ via diff($\Psi$), $\partial \eta/\partial z_{max}$=',...
        num2str(deta_dz_max)],['$J$=',num2str(J),...
        ', $\epsilon$=',num2str(epsilon)]},'Interpreter','latex')
    ylabel('z/Lc','Interpreter','latex')
    set(gca,'XTickLabel','')
    colorbar
...
%% saving velocity vectors for sharing
if saving==1
    uu=udiffint+U;
    ww=wdiffint;
    save('vels_witch_Jp8','uu','ww','dx','dz')
end